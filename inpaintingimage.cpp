...
InpaintingImage::InpaintingImage(cv::Mat inputImage,cv::Mat mask, cv::Mat croppedImage,int halfPatchWidth,int mode){
...
int InpaintingImage::checkValidInputs(){
...
void InpaintingImage::inpaint(){

    cv::namedWindow("updatedMask");
    cv::namedWindow("inpaint");
    cv::namedWindow("gradientX");
    cv::namedWindow("gradientY");
    cv::namedWindow("croppedImage");
    cv::namedWindow("sourceRegion");
    cv::namedWindow("OriginalsourceRegion");
    ...
    while(stay){
        ...
        cv::imshow("sourceRegion",sourceRegion);
        cv::imshow("OriginalsourceRegion",originalSourceRegion);
        cv::waitKey(2);
    }
...
void InpaintingImage::calculateGradients(){
    cv::Mat srcGray;
...
void InpaintingImage::initializeMats(){
...
void InpaintingImage::computeFillFront(){
...
void InpaintingImage::computeConfidence(){
    cv::Point2i a,b;
...
void InpaintingImage::computeData(){
...
void InpaintingImage::computeTarget(){
...
void InpaintingImage::computeBestPatch(){
    ...
    for(int x=0;x<=croppedImage.cols-width;x++){
        for(int y=0;y<=croppedImage.rows-height;y++){
            ...
            for(int x2=0;x2<width;x2++){
                for(int y2=0;y2<height;y2++){
                    ...
                    sourcePixel=croppedImage.at<cv::Vec3b>(y+y2,x+x2);
...
void InpaintingImage::updateMats(){
    ...
    for(int x=0;x<width;x++){
        for(int y=0;y<height;y++){
            if(sourceRegion.at<uchar>(a.y+y,a.x+x)==0){
                workImage.at<cv::Vec3b>(a.y+y,a.x+x)=croppedImage.at<cv::Vec3b>(bestMatchUpperLeft.y+y,bestMatchUpperLeft.x+x);
...
void InpaintingImage::getPatch(cv::Point2i &centerPixel, cv::Point2i &upperLeft, cv::Point2i &lowerRight){
    int x,y;
    x=centerPixel.x;
    y=centerPixel.y;
