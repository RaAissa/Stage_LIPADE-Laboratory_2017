...
void Video::creerVideo(string nomChemin, string nomFichier, int tailleVideo, string nouveauDossier, string traitement, Size frameSize){
    ...
    cout << "write to folder : " << nomChemin + "/" + nomFichier + "_" + traitement + ".avi" << " " << frameSize.width << " " << frameSize.height << endl;
    ...
    if (oVideoWriter.isOpened() && tailleVideo > 1)
    {
        for(int i = 1 ; i < tailleVideo ; i++)
        {
            ...
            Mat img = imread(cv::format((nouveauDossier + "/" + traitement + "/Kmeans2d/frame%d.tif").c_str(), i).c_str(), CV_LOAD_IMAGE_COLOR);
